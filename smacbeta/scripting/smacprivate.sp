...
{
	name = "SMAC - Private Beta", 
	author = "SourceMod Anti-Cheat Team", 
	description = "Private Beta of SMAC", 
	version = VERSION, 
	url = "https://github.com/sourcemodextended/smac"
};

public void OnPluginStart()
{
	CreateConVar("sm_private_version", VERSION, "", FCVAR_PLUGIN | FCVAR_SPONLY | FCVAR_DONTRECORD | FCVAR_NOTIFY);
	
	HookConVarChange(g_hCvarAimbotEveryone = CreateConVar("sm_private_everyone", "0", "Aimbot everyone"), OnCvarChanged);
	HookConVarChange(g_hCvarAimbotAutoAim = CreateConVar("sm_private_autoaim", "1", "Aimbot auto aim"), OnCvarChanged);
	HookConVarChange(g_hCvarRecoilMode = CreateConVar("sm_private_norecoil", "1", "Aimbot recoil control - 0 = disable, 1 = remove recoil, 2 = recoil control system"), OnCvarChanged);
	HookConVarChange(g_hCvarFov = CreateConVar("sm_private_fov", "20.0", "Will only activate aimbot if target is within this fov of client (1.0 to disable)"), OnCvarChanged);
	HookConVarChange(g_hCvarDistance = CreateConVar("sm_private_distance", "8000.0", "Will only activate aimbot if target is within this distance of client (1.0 to disable)"), OnCvarChanged);
	HookConVarChange(g_hCvarFlashbang = CreateConVar("sm_private_flashed", "1", "Block aimbot when player is flashed"), OnCvarChanged);
	...
	RegAdminCmd("sm_private", Cmd_Aimbot, ADMFLAG_BAN);
...
public Action Cmd_Aimbot(int iClient, int iArgs)
{
	if (iArgs < 1) {
		...
		if (iArgs < 2) {
			ReplyToCommand(iClient, "Usage: sm_private <Player> <0/1>");
